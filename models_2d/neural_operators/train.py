...
from dexsy_core.forward_model import ForwardModel2D, compute_dei, create_forward_model
...
def train_model(
    ...
    with torch.no_grad():
        if model_type == "fno":
            sample_input = torch.randn(2, 3, forward_model.n_b, forward_model.n_b).to(device)
        else:
            sample_input = torch.randn(2, 1, forward_model.n_b, forward_model.n_b).to(device)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train Neural Operator models on 2D DEXSY")
    parser.add_argument("--model_type", type=str, default="deeponet", choices=["deeponet", "fno"])
