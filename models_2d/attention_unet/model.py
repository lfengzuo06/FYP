...
def get_model(base_filters: int = 32, in_channels: int = 3) -> nn.Module:
    """
    Factory function to create the Attention U-Net model.
    ...
    Returns:
        AttentionUNet2D model
    """
    return AttentionUNet2D(in_channels=in_channels, base_filters=base_filters)
...
if __name__ == "__main__":
    ...
    model = AttentionUNet2D(in_channels=3, base_filters=32).to(device)
    print(f"\nAttentionUNet2D:")
