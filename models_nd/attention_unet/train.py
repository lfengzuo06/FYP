"""
Training script for Attention U-Net on N-Compartment DEXSY.

Usage:
    python -m models_nd.attention_unet.train

Or import and use programmatically:
    from models_nd.attention_unet import train_model
...
class DEXSYDatasetND(Dataset):
    """PyTorch Dataset for N-Compartment DEXSY data."""
...
def generate_dataset(
    ...
    """Generate train/val/test datasets for N-Compartment training."""
    set_seed(seed)
...
def train_model(
    ...
    Args:
        ...
        n_compartments: Number of compartments (2 or more)
    ...
    # Loss and optimizer
    criterion = PhysicsInformedLossND(
        forward_model=forward_model,
        alpha_kl=1.0,
        alpha_rec=0.2,
        alpha_signal=0.1,
    ...
    # Data loaders
    train_dataset = DEXSYDatasetND(
        datasets['train']['inputs'],
        datasets['train']['labels'],
        datasets['train']['clean_signals'],
        augment=True,
    )
    val_dataset = DEXSYDatasetND(
        datasets['val']['inputs'],
        datasets['val']['labels'],
    ...
    print(f"\nTraining Attention U-Net on {n_compartments}C data...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Attention U-Net on N-Compartment DEXSY")
