...
This implements the CNN baseline for 3C data:
...
class PlainUNet3C(nn.Module):
    """
    Plain U-Net for 3-Compartment DEXSY Inversion.
...
class PlainUNetLoss3C(nn.Module):
    """
    Loss function for Plain U-Net on 3C data.
...
def get_model(base_filters: int = 32, in_channels: int = 3) -> nn.Module:
    """
    Factory function to create the Plain U-Net model for 3C data.
    ...
    Returns:
        PlainUNet3C model
    """
    return PlainUNet3C(in_channels=in_channels, base_filters=base_filters)
...
if __name__ == "__main__":
    ...
    model = PlainUNet3C(in_channels=3, base_filters=32).to(device)
    print(f"\nPlainUNet3C (CNN Baseline for 3C):")
    ...
    # Compare with Attention U-Net 3C
    from models_3d.attention_unet import AttentionUNet3C
    attn_model = AttentionUNet3C(in_channels=3, base_filters=32).to(device)
    n_params_attn = sum(p.numel() for p in attn_model.parameters())
    print(f"\n  Attention U-Net 3C parameters: {n_params_attn:,}")
