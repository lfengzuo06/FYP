...
class ISTAProximalStep(nn.Module):
    ...
    def __init__(
        ...
        self.log_threshold = nn.Parameter(torch.tensor(-8.0))
...
class DeepUnfolding3C(nn.Module):
    ...
    Input: (batch, 1, 64, 64) raw signal (for Deep Unfolding)
    ...
    def __init__(
        ...
        """
        Initialize Deep Unfolding model for 3C.
...
def get_model(
    ...
    **kwargs,
    ...
    """
    Factory function to create a Deep Unfolding model for 3C.
    ...
    Returns:
        DeepUnfolding3C model
    """
    return DeepUnfolding3C(
        n_layers=n_layers,
        n_d=n_d,
...
if __name__ == "__main__":
    ...
    model = DeepUnfolding3C(n_layers=12, n_d=64, hidden_dim=256).to(device)
    print(f"\nDeepUnfolding3C (ISTA-Net style for 3C):")
    print(f"  Input: (batch, 1, 64, 64) raw signal")
    print(f"  Output: (batch, 1, 64, 64) normalized distribution")

    # Test forward pass
    x = torch.randn(2, 1, 64, 64).to(device)
    # Set dummy kernel for testing
    import numpy as np
    from dexsy_core.forward_model import ForwardModel2D
    fm = ForwardModel2D(n_d=64, n_b=64)
    K = torch.from_numpy(fm.kernel_matrix.astype(np.float32)).float().to(device)
    model.set_kernel_matrix(K)
