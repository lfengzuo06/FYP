"""
Inference pipeline for Attention U-Net on 3-Compartment DEXSY.

This module provides the standard inference interface for the Attention U-Net 3C model,
...
from .model import AttentionUNet3C


@dataclass
class PredictionResult3C:
...
class InferencePipeline3C:
    """
    Standard inference pipeline for Attention U-Net 3C.

    This class implements the unified interface required by the benchmark framework:
    - __init__(checkpoint_path=None)
    - predict(signal) -> PredictionResult3C
    - get_model_name() -> str
    - get_model_info() -> dict
    """

    MODEL_NAME = "attention_unet_3c"
    ...
    def _find_default_checkpoint(self) -> Path | None:
        ...
        possible_paths = [
            Path(__file__).parent.parent.parent / "checkpoints" / "attention_unet_3c" / "best_model.pt",
            Path(__file__).parent.parent.parent / "training_output_3d" / "attention_unet_3c" / "checkpoints_latest" / "best_model.pt",
        ]
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            **self.model_metadata,
            "model_type": "Attention U-Net 3C",
            "description": "U-Net with attention gates for 3-compartment DEXSY",
    ...
    def predict_batch(
        ...
        *,
        ...
        for idx, reconstructed in enumerate(predictions):
            ...
            results.append(PredictionResult3C(
                signal=signal_2d,
                reconstructed_spectrum=reconstructed,
                dei=float(summary["dei"]),
                summary_metrics=summary,
...
def predict(
    ...
    *,
    ...
    """
    High-level inference function for Attention U-Net 3C.
    ...
    Returns:
        PredictionResult3C with prediction details
    """
...
def predict_batch(
    ...
    *,
    ...
    """
    High-level batch inference function for Attention U-Net 3C.
...
if __name__ == "__main__":
    ...
    print("Testing InferencePipeline3C...")
